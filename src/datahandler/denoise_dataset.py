...
from ..util.util import rot_hflip_img, tensor2np, np2tensor, mean_conv2d, cal_sensitivity, calculate_noise_scale


class DenoiseDataSet(Dataset):
    def __init__(self,dataset_dir: str = None, add_noise: str = None, crop_size: list = None, random_unshuffling_f=None, aug: list = None,
                 n_repeat: int = 1, n_data: int = None, ratio_data: float = None) -> None:
        '''
    ...
    def _add_noise(self, clean_img: torch.Tensor, add_noise_type: str, opt: list, clamp: bool = False) -> torch.Tensor:
        ...
        if add_noise_type == 'bypass':
            ...
            # add AWGN
            nlf = opt[0]
            sensitivity = cal_sensitivity(0.001, 10, 8)
            noise_scale = calculate_noise_scale('Gaussian', opt[1], 0.0001, 8)
            nlf = sensitivity*noise_scale
...
class MergedDataset(Dataset):
    def __init__(self, d_list, *args, **kwargs):
        ...
        self.dataset_list = []
        for d in d_list:
            self.dataset_list.append(get_dataset_class(d)(*args, **kwargs))
